...
@section('content') 
@section('headmsg') 
Sales Report between <?php echo Input::get('datepickerFrom')." and ".Input::get('datepickerTo'); ?> 
@endsection 
...
//  	echo $_POST('datepickerFrom');
  	...
  	$today=date('Y-m-d');
  	$fromdate=date(Input::get('datepickerFrom'));
  	$todate=date(Input::get('datepickerTo'));
    ...
    $sql=mysql_query("select sum(total_paid) from sales where created_at between '$fromdate' and '$todate 23:59:59'"); 
    ...
    $sql=mysql_query("select sum(credit) from sales where created_at between '$fromdate' and '$todate 23:59:59'"); 
    while($lrow = mysql_fetch_array($sql))  
    { 
        $sumcredit = $lrow['sum(credit)'];   
	} 

    $sql=mysql_query("select sum(total_paid) from dailypurchases where purchase_date between '$fromdate' and '$todate 23:59:59'");
    while($lrow = mysql_fetch_array($sql))
    {
    	$purchasepaid = $lrow['sum(total_paid)'];
    }

    $sql=mysql_query("select sum(total_cost) from dailypurchases where purchase_date between '$fromdate' and '$todate 23:59:59'");
    while($lrow = mysql_fetch_array($sql))
    {
    	$purchasecost = $lrow['sum(total_cost)'];
    }

    $sql=mysql_query("select sum(discount) from sales where created_at between '$fromdate' and '$todate 23:59:59'"); 
    while($lrow = mysql_fetch_array($sql))  
    { 
   	...
   	$sql=mysql_query("select sum(total_credit) from dailypurchases where purchase_date between '$fromdate' and '$todate 23:59:59'");
    while($lrow = mysql_fetch_array($sql))
    {
    ...
    $sql=mysql_query("select sum(sp) from dailysales where sale_date between '$fromdate' and '$todate 23:59:59'");
    while($lrow = mysql_fetch_array($sql))
    {
        $totalsp = $lrow['sum(sp)'];
    }

    $sql=mysql_query("select sum(cp) from dailysales where sale_date between '$fromdate' and '$todate 23:59:59'");
    while($lrow = mysql_fetch_array($sql))
    {
        $totalcp = $lrow['sum(cp)'];
    }
